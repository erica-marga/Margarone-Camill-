...
//il browser vede la data come se fosse una stringa di testo. prima di inserirla nel db devo trasformarla in un dato di tipo data.
//la funzione strtotime esegue questa conversione. dopo aver trasformato la stringa in data, devo formattarla nello standard anno-mese-giorno.
//la funzione date serve a formattare la data nel modo idoneo per essere memorizzata nel database mysql
$miadata= date("Y-m-d", strtotime($data));
		$sql= "INSERT INTO tbl_utenti (username,password,nome,cognome,data_nascita,mail) VALUES ('$username','$password','$nome','$cognome','$miadata','$email')";
//$sql= "INSERT INTO tbl_utenti (username,password,nome,cognome,mail) VALUES ('prova','12345','ciccio','conlapalla','fa@tim.it')";

		if ($conn->query($sql) === TRUE) {
   echo "<p class=\"text-center\"><span class=\"label label-info\">UTENTE REGISTRATO CORRETTAMENTE</span></p><br>";
} else {
